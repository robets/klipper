...
from common.smoothing import window_average
...
class Heater:
    def __init__(self, config, sensor):
        ...
        self.smooth_time = config.getfloat('smooth_time', 1., minval=0.)
        self.smoothing = None
        self._set_smoothing(window_average, math.ceil(self.smooth_time / self.sensor.get_report_time_delta()))
    ...
    def _set_smoothing(self, smoothing_function, *args):
        self.smoothing = smoothing_function(*args)
    ...
    def temperature_callback(self, read_time, temp):
        with self.lock:
            # Raw temperature tracking
            sample = SensorSample(value=temp, time=read_time)
            try:
                self.smoothed_temp = self.smoothing.send(sample)
            except StopIteration as e:
                logging.warning(e)
                raise e

            # Smoothed temperature tracking
    ...
    def check_busy(self, eventtime):
        """
        Check if heater is busy reaching target temperature, or has stabilized.
        :param eventtime: ?
        :return: Boolean true if heater is not stabilized. False if heater is stablized at target.
        """
        with self.lock:
...
class ControlPID:
    ...
    def check_busy(self, eventtime, smoothed_temp, target_temp):
        ...
        Check if heater is busy reaching target temperature, or has stabilized.
        :param eventtime: ?
        :param smoothed_temp: The post-filter temperature to check against target.
